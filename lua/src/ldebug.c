...
static int precheck (const Proto *pt) {
  ...
  check(GET_OPCODE(pt->code[pt->sizecode-1]) == HKS_OPCODE_RETURN);
  return 1;
}
...
static Instruction symbexec (const Proto *pt, int lastpc, int reg) {
  ...
  for (pc = 0; pc < lastpc; pc++) {
      ...
      case iAsBx: {
        ...
        if (getBMode(op) == OpArgR) {
          ...
          if (dest > 0) {
            ...
            check(!(GET_OPCODE(d) == HKS_OPCODE_SETLIST && GETARG_C(d) == 0));
          }
        }
        break;
      }
    ...
    if (testTMode(op)) {
      ...
      check(GET_OPCODE(pt->code[pc+1]) == HKS_OPCODE_JMP);
    }
    switch (op) {
      ...
      case HKS_OPCODE_CLOSURE: {
        ...
        for (; nup>0; nup--) {
          ...
          check(op1 == HKS_OPCODE_GETUPVAL || op1 == HKS_OPCODE_MOVE);
        }
        break;
      }
...
static const char *getfuncname (lua_State *L, CallInfo *ci, const char **name) {
  ...
  if (GET_OPCODE(i) == HKS_OPCODE_CALL || GET_OPCODE(i) == HKS_OPCODE_TAILCALL ||
      GET_OPCODE(i) == HKS_OPCODE_TFORLOOP)
